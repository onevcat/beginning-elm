...
fetchPost postId =
    Http.get
        ...
        , expect = postDecoder |> Http.expectJson (RemoteData.fromResult >> PostReceived)
        }
...
update msg model =
    case msg of
        ...
        UpdateTitle newTitle ->
            let
                updateTitle =
                    RemoteData.map (\postData -> { postData | title = newTitle }) model.post
            in
        ...
        UpdateAuthorName newAuthorName ->
            let
                updateAuthorName =
                    RemoteData.map (\postData -> { postData | authorName = newAuthorName }) model.post
            in
        ...
        UpdateAuthorUrl newAuthorUrl ->
            let
                updateAuthorUrl =
                    RemoteData.map (\postData -> { postData | authorUrl = newAuthorUrl }) model.post
            in
